...
import { gerarFluxogramaWeb } from '../../../fontes/visoes/fluxogramas/geracao-fluxogramas-web';
...
const mockUri = {
    fsPath: '/projeto/arquivo.delegua',
    path: '/projeto/arquivo.delegua'
} as any;
...
describe('gerarFluxogramaWeb', () => {
    beforeEach(() => {
        (vscode.workspace.fs.readFile as jest.Mock).mockResolvedValue(
            new Uint8Array(Buffer.from('var x = 1'))
        );

        mockImportadorInstance = {
            importarViaFuncaoConteudoDocumento: jest.fn().mockReturnValue({
    ...
    describe('validação de URI', () => {
        it('exibe erro quando uri é undefined e sem editor ativo', async () => {
            await gerarFluxogramaWeb(undefined, mockContext);
    ...
    describe('validação de extensão', () => {
        it('não exibe aviso para arquivo .delegua', async () => {
            await gerarFluxogramaWeb(mockUri, mockContext);
            expect(vscode.window.showWarningMessage).not.toHaveBeenCalled();
        });

        it('não exibe aviso para arquivo .delégua', async () => {
            const uri = { fsPath: '/projeto/programa.delégua', path: '/projeto/programa.delégua' } as any;
            await gerarFluxogramaWeb(uri, mockContext);
            expect(vscode.window.showWarningMessage).not.toHaveBeenCalled();
        });

        it('exibe aviso para arquivo com extensão inválida', async () => {
            const uri = { fsPath: '/projeto/script.js', path: '/projeto/script.js' } as any;
            await gerarFluxogramaWeb(uri, mockContext);
    ...
    describe('leitura de arquivo via workspace.fs', () => {
        it('lê arquivo via vscode.workspace.fs.readFile', async () => {
            await gerarFluxogramaWeb(mockUri, mockContext);
            expect(vscode.workspace.fs.readFile).toHaveBeenCalledWith(mockUri);
        });

        it('usa importarViaFuncaoConteudoDocumento em vez de importar', async () => {
            await gerarFluxogramaWeb(mockUri, mockContext);
            expect(mockImportadorInstance.importarViaFuncaoConteudoDocumento).toHaveBeenCalled();
            expect(mockImportadorInstance.importar).toBeUndefined();
        });

        it('extrai nome de arquivo do path via split em vez de path.basename', async () => {
            await gerarFluxogramaWeb(mockUri, mockContext);
    ...
    describe('geração do fluxograma', () => {
        ...
        it('passa retornoLexador do importador para o avaliador', async () => {
            ...
            mockImportadorInstance.importarViaFuncaoConteudoDocumento.mockReturnValue({ retornoLexador });
            await gerarFluxogramaWeb(mockUri, mockContext);
    ...
    describe('tratamento de erros', () => {
        it('exibe mensagem de erro quando exceção é lançada', async () => {
            (vscode.window.withProgress as jest.Mock).mockRejectedValue(new Error('Falha ao ler'));
            await gerarFluxogramaWeb(mockUri, mockContext);
        ...
        it('não propaga exceção para o chamador', async () => {
            ...
            await expect(gerarFluxogramaWeb(mockUri, mockContext)).resolves.not.toThrow();
        });
    });
});
