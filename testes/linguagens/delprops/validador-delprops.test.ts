...
describe('validarDelprops', () => {
    ...
    // ──── propriedades obrigatórias (sem padrao) ────

    function contribuicao(definicoes: any[]) {
        return [{ pacote: '@designliquido/vscode', definicoes }];
    }

    it('propriedade sem padrao ausente → Warning', () => {
        analisar.mockReturnValue({
            propriedades: [{ chave: 'liquido.aplicacao.nome', valor: "'MeuApp'", linha: 1 }],
            erros: [],
        });
        validar.mockReturnValue({ avisos: [], erros: [] });
        obterTodos.mockReturnValue(new Map([
            ['liquido.aplicacao', contribuicao([
                { nome: 'nome', tipo: 'texto', detalhe: 'Nome do projeto' },
                { nome: 'versao', tipo: 'texto', detalhe: 'Versão' },
            ])],
        ]));

        const diags = validarDelprops(criarDocumento(["liquido.aplicacao.nome = 'MeuApp'"]));
        expect(diags).toHaveLength(1);
        expect(diags[0].message).toContain('versao');
        expect(diags[0].message).toContain('obrigatória');
        expect(diags[0].severity).toBe(1);
        expect(diags[0].range.start.line).toBe(0);
        expect(diags[0].range.start.character).toBe(0);
    });

    it('propriedade com padrao não é considerada obrigatória → sem Warning', () => {
        analisar.mockReturnValue({
            propriedades: [{ chave: 'liquido.roteador.cors', valor: 'verdadeiro', linha: 1 }],
            erros: [],
        });
        validar.mockReturnValue({ avisos: [], erros: [] });
        obterTodos.mockReturnValue(new Map([
            ['liquido.roteador', contribuicao([
                { nome: 'cors', tipo: 'logico', detalhe: 'Habilita CORS', padrao: 'verdadeiro' },
                { nome: 'porta', tipo: 'numero', detalhe: 'Porta', padrao: '3000' },
            ])],
        ]));
    ...
    it('todas as obrigatórias preenchidas → sem Warning', () => {
        analisar.mockReturnValue({
            propriedades: [
                { chave: 'liquido.aplicacao.nome', valor: "'App'", linha: 1 },
                { chave: 'liquido.aplicacao.versao', valor: "'1.0.0'", linha: 2 },
            ],
            erros: [],
        });
        validar.mockReturnValue({ avisos: [], erros: [] });
        obterTodos.mockReturnValue(new Map([
            ['liquido.aplicacao', contribuicao([
                { nome: 'nome', tipo: 'texto', detalhe: 'Nome' },
                { nome: 'versao', tipo: 'texto', detalhe: 'Versão' },
            ])],
        ]));

        const diags = validarDelprops(criarDocumento([
            "liquido.aplicacao.nome = 'App'",
            "liquido.aplicacao.versao = '1.0.0'",
        ]));
        expect(diags).toHaveLength(0);
    });

    it('liquido.dados com fonte definida mas propriedade obrigatória ausente → Warning', () => {
        analisar.mockReturnValue({
            propriedades: [{ chave: 'liquido.dados.bd.tecnologia', valor: "'sqlite'", linha: 1 }],
            erros: [],
        });
        validar.mockReturnValue({ avisos: [], erros: [] });
        obterTodos.mockReturnValue(new Map([
            ['liquido.dados', contribuicao([
                { nome: 'tecnologia', tipo: 'texto', detalhe: 'Tecnologia' },
                { nome: 'caminho', tipo: 'texto', detalhe: 'Caminho' },
            ])],
        ]));

        const diags = validarDelprops(criarDocumento(["liquido.dados.bd.tecnologia = 'sqlite'"]));
        expect(diags).toHaveLength(1);
        expect(diags[0].message).toContain('caminho');
        expect(diags[0].message).toContain('liquido.dados.bd');
    });

    it('liquido.dados com fonte definida e todas obrigatórias preenchidas → sem Warning', () => {
        analisar.mockReturnValue({
            propriedades: [
                { chave: 'liquido.dados.bd.tecnologia', valor: "'sqlite'", linha: 1 },
                { chave: 'liquido.dados.bd.caminho', valor: "':memory:'", linha: 2 },
            ],
            erros: [],
        });
        validar.mockReturnValue({ avisos: [], erros: [] });
        ...
        const diags = validarDelprops(criarDocumento([
            "liquido.dados.bd.tecnologia = 'sqlite'",
            "liquido.dados.bd.caminho = ':memory:'",
        ]));
        expect(diags).toHaveLength(0);
    });

    it('vários namespaces com obrigatórias → múltiplos Warnings', () => {
        analisar.mockReturnValue({
            propriedades: [],
            erros: [],
        });
        validar.mockReturnValue({ avisos: [], erros: [] });
        obterTodos.mockReturnValue(new Map([
            ['liquido', contribuicao([
                { nome: 'arquetipo', tipo: 'texto', detalhe: 'Arquétipo' },
            ])],
            ['liquido.aplicacao', contribuicao([
                { nome: 'nome', tipo: 'texto', detalhe: 'Nome' },
                { nome: 'versao', tipo: 'texto', detalhe: 'Versão' },
        ...
        const diags = validarDelprops(criarDocumento([]));
        expect(diags).toHaveLength(3);
        expect(diags.filter(d => d.severity === 1)).toHaveLength(3);
    });
